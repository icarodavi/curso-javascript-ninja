...
//  username = username ? username : 'Desconhecido';
  ...
  */
  ...
  `$inputUsername`.
  */
  var $inputUsername = doc.querySelectorAll('input[type="text"]');//doc.querySelector('[name="Nome"]');
  ...
  */
  $button.addEventListener('click', function(event) {
    ...
    if (!isValidEmail($inputEmail.value)){
      return alert('Preencha com email válido');
    }
    ...
    if($message.value && $inputEmail.value && $inputUsername.value) {
      var confirma = confirm('Deseja enviar o formulário?');
      if (!confirma) {
  ...
  */
  function isValidEmail(email) {
    return /^[\w+.]+@\w+?\.\w{2,}(?:\.\w{2})?$/gi.test(email);
  }

  var arr = [ 'meu.email+categoria@gmail.com', 'juca_malandro@bol.com.br', 'pedrobala@hotmail.uy', 'sandro@culinaria.dahora', 'walter-da-silva@maraca.br', 'rita-marica@titica.a.b', 'agua_@evida.br.com' ]
